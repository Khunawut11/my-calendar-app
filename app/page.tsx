  ...
  const monthsOverview = useMemo(() => {
    ...
    return months.map(m => {
      ...
      if (totalPaintedDays > 0) {
        ...
        if (busyRatio >= 0.4) {
           color = 'bg-rose-500 text-white border-rose-600 shadow-md scale-[1.02]'; 
        } else if (busyRatio > 0.1) {
           color = 'bg-rose-200 text-rose-800 border-rose-300'; 
        } else if (availRatio >= 0.7) {
           color = 'bg-emerald-500 text-white border-emerald-600 shadow-md scale-[1.02]'; 
        } else if (availRatio >= 0.3) {
           color = 'bg-emerald-300 text-emerald-900 border-emerald-400'; 
        } else {
    ...
    <div className="min-h-screen bg-[#f1f5f9] p-2 md:p-8 font-sans text-slate-800 select-none">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
            ...
            <div className="bg-white p-5 md:p-6 rounded-[2rem] md:rounded-[2.5rem] shadow-xl border border-white/50 relative">
              ...
              <div className="mb-6 md:mb-8">
                ...
                <div className="flex gap-2 mb-3 md:mb-4">
                  <input 
                    type="text" value={newMemberName} onChange={(e) => setNewMemberName(e.target.value)} placeholder="Name..."
                    className="flex-1 bg-slate-50 border-none rounded-xl px-3 py-2 md:px-4 text-sm focus:ring-2 ring-emerald-400 outline-none w-full"
                  />
        ...
        <div className={isViewerMode ? "lg:col-span-12 space-y-4 md:space-y-6 max-w-5xl mx-auto w-full" : "lg:col-span-9 space-y-4 md:space-y-6"}>
          ...
          {isViewerMode && (
            <div className="bg-emerald-100 text-emerald-800 p-3 md:p-4 rounded-xl md:rounded-2xl font-bold flex items-center justify-center gap-2 shadow-sm border border-emerald-200 text-sm md:text-base text-center">
              <Share2 size={18} className="shrink-0" /> You are viewing this calendar in Read-Only mode.
            </div>
          )}
            ...
            <div className="flex-1 bg-slate-900 p-5 md:p-6 rounded-[2rem] md:rounded-[2.5rem] text-white shadow-2xl flex flex-col h-56 md:h-64 border border-slate-700">
              ...
              <div className="overflow-y-auto pr-1 md:pr-2 space-y-2 custom-scrollbar">
                {allValidRanges.length > 0 ? (
                  allValidRanges.map((r, idx) => {
                    ...
                    return (
                      <div key={idx} onClick={() => setCurrentDate(r.startDate)} 
                        className={`flex justify-between items-center cursor-pointer group transition-all duration-300 rounded-xl md:rounded-2xl px-3 md:px-4 py-2 md:py-3 ${isTopMatch ? 'bg-white/10 scale-[1.02] border border-white/20' : 'hover:bg-white/5 border-b border-white/5'}`}>
                        <div className="flex items-center gap-2 md:gap-3">
                          {isTopMatch ? <Trophy size={16} className="text-yellow-400 md:w-[18px] md:h-[18px]" /> : <ArrowRightCircle size={14} className="text-slate-600 group-hover:text-emerald-400 transition md:w-[16px] md:h-[16px]" />}
                          <span className={`text-base md:text-xl font-black tracking-tight ${isTopMatch ? 'text-yellow-400' : 'text-slate-200 group-hover:text-white'}`}>{r.range}</span>
                        </div>
                        <span className={`text-[8px] md:text-[10px] font-black uppercase px-2 md:px-3 py-1 md:py-1.5 rounded-full whitespace-nowrap ${isEveryone ? 'bg-yellow-400 text-yellow-900 shadow-[0_0_15px_rgba(250,204,21,0.4)]' : (isTopMatch ? 'bg-blue-500 text-white shadow-[0_0_10px_rgba(59,130,246,0.5)]' : 'bg-emerald-500/20 text-emerald-400')}`}>
                  ...
                  <div className="text-slate-500 text-xs md:text-sm italic mt-4 text-center">No available periods for {tripDays} days...</div>
                )}
              </div>
            </div>
                 ...
                 {monthsOverview.map((m, i) => {
                   ...
                   return (
                     <div key={i} onClick={() => setCurrentDate(m.date)} 
                       className={`w-9 h-9 md:w-12 md:h-12 rounded-lg md:rounded-xl flex flex-col items-center justify-center transition-all cursor-pointer border hover:ring-2 hover:ring-emerald-400
          ...
          <div className="bg-white p-4 sm:p-6 md:p-10 rounded-[2rem] md:rounded-[3.5rem] shadow-2xl shadow-slate-200 border border-white">
            <div className="flex items-center justify-between mb-6 md:mb-12">
              ...
              <div className="flex gap-1 md:gap-3 bg-slate-50 p-1 md:p-2 rounded-xl md:rounded-2xl">
                <button onClick={()=>setCurrentDate(subMonths(currentDate, 1))} className="p-2 md:p-3 hover:bg-white rounded-lg md:rounded-2xl transition shadow-sm"><ChevronLeft size={20} className="md:w-[28px] md:h-[28px]" /></button>
                <button onClick={()=>setCurrentDate(addMonths(currentDate, 1))} className="p-2 md:p-3 hover:bg-white rounded-lg md:rounded-2xl transition shadow-sm"><ChevronRight size={20} className="md:w-[28px] md:h-[28px]"/></button>
              </div>
            </div>

            <div className="grid grid-cols-7 gap-1 sm:gap-2 md:gap-4">
              {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((d, i) => (
                <div key={d} className="text-center text-[8px] sm:text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest md:tracking-[0.4em] mb-2 md:mb-4 pointer-events-none">
              ...
              {Array.from({ length: getDay(startOfMonth(currentDate)) }).map((_, i) => (
                <div key={`empty-${i}`} className="h-16 sm:h-24 md:h-32 lg:h-40 pointer-events-none" />
              ))}
              ...
              {daysInMonth.map(day => {
                ...
                if (isEveryone) cellBg = 'bg-gradient-to-br from-yellow-300 to-amber-500 border-yellow-400 shadow-[0_0_15px_rgba(251,191,36,0.3)] md:shadow-[0_0_30px_rgba(251,191,36,0.5)] md:scale-105 z-10';
                ...
                return (
                  <div key={dateKey} onMouseDown={() => { if(!isViewerMode) { setIsDragging(true); applyStatus(dateKey); } }} onMouseEnter={() => { if(!isViewerMode && isDragging) applyStatus(dateKey); }}
                    className={`h-16 sm:h-24 md:h-32 lg:h-40 p-1 sm:p-2 md:p-4 rounded-xl md:rounded-[2rem] transition-all duration-300 border-2 relative overflow-hidden flex flex-col justify-start md:justify-between group
                      ${isViewerMode ? 'cursor-default' : 'cursor-pointer'} ${cellBg}`}>
                    
                    <span className={`text-sm sm:text-lg md:text-2xl font-black pointer-events-none text-center md:text-left ${isEveryone ? 'text-white' : (availableCount > 0 ? 'text-emerald-700' : busyCount > 0 ? 'text-rose-400' : 'text-slate-300')}`}>
                      {format(day, 'd')}
                    </span>
                    
                    <div className="flex flex-wrap justify-center md:justify-start gap-0.5 md:gap-1 pointer-events-none mt-1 md:mt-0">
                      {members.map(m => {
                        const status = dayData[m.id];
                        if (!status) return null;
                        const isAvail = status === 'available';
                        return (
                          <div key={m.id} className={`w-3.5 h-3.5 sm:w-5 sm:h-5 md:w-6 md:h-6 rounded-md md:rounded-lg backdrop-blur-md flex items-center justify-center text-[5px] sm:text-[7px] md:text-[8px] font-black border text-white shadow-sm
                            ${isAvail ? 'bg-emerald-500 border-emerald-400' : 'bg-rose-500 border-rose-400'}`}>{m.short}</div>
                        );
                      })}
                    </div>
                    {isEveryone && <Sparkles className="absolute top-1 right-1 md:top-4 md:right-4 text-white animate-pulse pointer-events-none w-3 h-3 md:w-4 md:h-4" />}
                  </div>
                );
              })}
            </div>
